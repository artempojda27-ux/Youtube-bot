...
async function callClaude(systemPrompt: string, userPrompt: string): Promise<string> {
  ...
  // content[0] не всегда текст — если Claude включает "размышление" перед
  // ответом, первым блоком идёт {type:"thinking"}, без поля .text. Тот же
  // баг ловили вчера в LEADS — ищем именно текстовый блок, не берём [0] вслепую.
  const textBlock = json.content?.find((b: any) => b.type === 'text');
  if (!textBlock?.text) throw new Error(`Claude вернул пустой ответ. stop_reason: ${json.stop_reason}, content: ${JSON.stringify(json.content)}`);
