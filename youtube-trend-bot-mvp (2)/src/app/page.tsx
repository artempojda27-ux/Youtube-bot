  ...
  async function handleGenerateVideo(id: string, fullScript: string) {
    ...
    try {
      ...
      if (!res.ok || data.error || !data.videoId) {
        setVideoJobs(prev => ({ ...prev, [id]: { status: 'error', error: data.error || 'Не удалось запустить рендер' } }));
        return;
      }
    ...
    // Опрашиваем статус САМИ, с браузера — не держим serverless-функцию
    // открытой минутами, у неё есть жёсткий лимит по времени выполнения.
    for (let i = 0; i < 40; i++) {
      ...
      try {
        ...
        if (statusData.status === 'completed' && statusData.videoUrl) {
          setVideoJobs(prev => ({ ...prev, [id]: { status: 'done', videoUrl: statusData.videoUrl } }));
          return;
        }
        if (statusData.status === 'failed' || statusData.error) {
          setVideoJobs(prev => ({ ...prev, [id]: { status: 'error', error: statusData.error || 'Рендер не удался' } }));
          return;
        }
        // status === 'processing' — просто ждём следующего опроса
      } catch (err: any) {
        // одиночный сбой сети при опросе — не сдаёмся сразу, пробуем ещё раз
        // на следующей итерации, только логируем на всякий случай
      }
    }
    setVideoJobs(prev => ({ ...prev, [id]: { status: 'error', error: 'Не дождались рендера за ~5 минут — попробуй позже.' } }));
  }
